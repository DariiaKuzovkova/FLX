...
function Employee(empl) {
	...
	this.getWorkTimeInSeconds = function() {
		...
		return Math.floor(workTimeInComp/toSec);
	}
...
function Company(comp) {
	...
	this.addNewEmployee = function(nameOfEmployee) {
		...
		if (nameOfEmployee instanceof Employee && this.employeesList.length < this.maxCompanySize && 
			...
			_logs.push( `${nameOfTheEmployee.name} starts working at ${this.name} 
				in ${nameOfTheEmployee.startDate} \n` );
		} else {
	...
	this.removeEmployee = function(id) {
		...
		_logs.push( `${this.employeesList[id].name} ends working at ${this.name} 
			in ${this.employeesList[id].endDate} \n` );
	...
	this.getAvarageSalary = function() {
		...
		return Math.round(_avarage*hundr)/hundr;
	}
